...
`tdx people` exposes TD's read-only people-discovery surfaces so you can
find UIDs, see who reports to whom, and look up the exact account / pool
names that `--account` and `--resource-pool` selectors expect.
