...
const lookupFixture = `[
	{"UID":"d44687e1-1a09-ef11-86d4-df13b8e4e655","FullName":"Iain Moffat","PrimaryEmail":"ipm@ufl.edu","IsActive":true,"IsEmployee":true,"DefaultAccountName":"14300000 (IT-ICT INFRA COMM TECHNOLOGY)","Title":"Director"},
...
func TestLookupPeople_QueryParamsAndDecode(t *testing.T) {
	...
	require.Equal(t, "Iain Moffat", users[0].FullName)
...
func TestLookupPeople_URLEncodesSearchText(t *testing.T) {
	...
	_, err := svc.LookupPeople(context.Background(), profile, "Iain Moffat", 10)
	require.NoError(t, err)
	require.True(t,
		strings.Contains(lastQuery, "searchText=Iain+Moffat") ||
			strings.Contains(lastQuery, "searchText=Iain%20Moffat"),
