...
func TestService_Rename_Success(t *testing.T) {
	...
	if err := s.store.SavePulledSnapshot(d); err != nil {
		t.Fatal(err)
	}
	if _, err := s.snapshots.Take(d, OpManual, ""); err != nil {
		t.Fatal(err)
	}

	if err := s.Rename("work", week, "old", "new"); err != nil {
		t.Fatal(err)
	}

	if !s.store.Exists("work", week, "new") {
		t.Errorf("new draft missing after rename")
	}
	if s.store.Exists("work", week, "old") {
		t.Errorf("old draft still present after rename")
	}
...
func TestService_Rename_RefusesCollision(t *testing.T) {
	...
	if !s.store.Exists("work", week, "a") {
		t.Errorf("source disappeared on failed rename")
	}
